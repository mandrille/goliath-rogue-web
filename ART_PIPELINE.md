...
## Re-rendering an edited spec

Editing an `art_src` spec changes nothing by itself. The PNG in `assets/art/` is
what the game loads, and for every hero, boss bone and character sprite it was
rendered by hand, once — neither batch builder covers them.

```bash
python tools/render_spec.py --changed     # whatever git says you edited
python tools/render_spec.py art_src/Heroes/fairy_idle.txt
godot --headless --path . --import        # or the old texture stays cached
```

The output size is the contract: the multiplier comes from the PNG already on
disk, not from the spec's `scale:` line, and the script refuses rather than
guesses when the grid no longer maps onto it. That refusal is the point —
`skeleter_jaw.txt` is a 16-wide grid with `mirror-x: even`, so the renderer emits
32, and dividing the shipped width by the grid width gives double the right
answer and a dislocated skull.
